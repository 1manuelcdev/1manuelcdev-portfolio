    ...
    return (
      <div className={cn("flex flex-row items-center gap-x-2 justify-center", className)}>
        ...
        <p className="text-primary text-sm">
          Upload concluído {files.length} arquivo{files.length > 1 ? "s" : ""}
        </p>
      </div>
    );
  }
    ...
    <div className={cn("flex flex-col", className)}>
          ...
          >
            ...
            <div className="shrink grow flex flex-col items-start truncate">
              ...
              {file.errors.length > 0 ? (
                  ...
                  {file.errors
                    .map((e) =>
                      e.message.startsWith("File is larger than")
                        ? `Arquivo maior que ${formatBytes(maxFileSize, 2)} (Tamanho: ${formatBytes(file.size, 2)})`
                        : e.message,
                    )
                    .join(", ")}
                </p>
                ...
                <p className="text-xs text-destructive">Falha no upload: {fileError.message}</p>
      ...
      {exceedMaxFiles && (
        <p className="text-sm text-left mt-2 text-destructive">
          Você pode enviar no máximo {maxFiles} arquivo{maxFiles > 1 ? "s" : ""}, remova{" "}
          {files.length - maxFiles} arquivo{files.length - maxFiles > 1 ? "s" : ""}.
        </p>
      )}
  ...
  return (
    <div className={cn("flex flex-col items-center gap-y-2", className)}>
      ...
      <p className="text-sm">
        ...
        >
          selecione{maxFiles === 1 ? " um arquivo" : " arquivos"}
        </a>
      </p>
      {maxFileSize !== Number.POSITIVE_INFINITY && (
        <p className="text-xs text-muted-foreground">
          Tamanho máximo: {formatBytes(maxFileSize, 2)}
        </p>
      )}
    </div>
  );
